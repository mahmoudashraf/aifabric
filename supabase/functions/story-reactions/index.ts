...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const { story_slug, action = "add" } = await req.json();
      ...
      if (action === "add") {
        ...
        console.log(`Removing reaction from story: ${story_slug}`);
        ...
        if (!reactions || reactions.length === 0) {
          // No reactions to remove - just return current count
      ...
      console.log(`Reaction ${action === "add" ? "added" : "removed"} successfully. New count: ${count}`);
